...

{
  shortblitoPackages =
    let
      ...
      shortblitoPkgWithComp =
        exeName: name:
        generateOptparseApplicativeCompletion exeName (shortblitoPkg name);
      shortblitoPkgWithOwnComp = name: shortblitoPkgWithComp name name;

    in
    {
      "shortblito-web-server" = shortblitoPkg "shortblito-web-server";
    };

  shortblitoRelease =
    final.symlinkJoin {
      name = "shortblito-release";
      paths = final.lib.attrValues final.shortblitoPackages;
    };
